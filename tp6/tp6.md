...
``````
[matis@dns ~]$ sudo cat /etc/named.conf
...
``````
[matis@dns ~]$ sudo cat /var/named/tp6.b1.db
$TTL 86400
...
``````
``````
[matis@dns ~]$ sudo cat /var/named/tp6.b1.rev
$TTL 84600
@ IN SOA dns.tp6.b1. admin.tp6.b1. (
    2018052900 ;Serial
    3200 ;Refresh
...
``````
[matis@dns ~]$ sudo systemctl status named
...
``````
[matis@dns ~]$ sudo firewall-cmd --add-port=53/udp --permanent
success
[matis@dns ~]$ sudo firewall-cmd --reload
success
``````
TEST
...
- Configuration
``````
[matis@john ~]$ sudo cat /etc/resolv.conf
[sudo] password for matis:
...
``````
PS C:\Users\celes> nslookup john.tp6.b1
Serveur :   UnKnown
Address:  10.6.1.101
...
``````
[matis@dhcp ~]$ sudo dnf install dhcp-server -y
Complete !
``````
``````
